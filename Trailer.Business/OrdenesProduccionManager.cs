    ...
    public class OrdenesProduccionManager : Repository<EE_ORDENES_PRODUCCION>
    {
        ...
        public DataPaged<EE_ORDENES_PRODUCCION> GetAllOrdenesProduccion(PagingInfo info, string Codigo)
        {
            ...
            else
            {
                ...
                {
                    int idcliente = Convert.ToInt32(info.search);
                    result = from p in context.EE_ORDENES_PRODUCCION
                             where p.ID_TIPO_CLIENTE == idcliente && p.CLIENTE != null && p.IMPORTE_FACTURADO < p.TOTAL
                             select p;
                }
                else if (Codigo == "FACTURAEMPRESA")
                {
                    int idcliente = Convert.ToInt32(info.search);
                    result = from p in context.EE_ORDENES_PRODUCCION
                             where p.ID_TIPO_CLIENTE == idcliente && p.EMPRESA != null && p.IMPORTE_FACTURADO < p.TOTAL
                             select p;
                }
                else
                {
