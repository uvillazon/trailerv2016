        ...
        public DataPaged<EE_CORTES> GetAllCortes(PagingInfo info, string Codigo)
        {
            ...
            if ((info.search == null) || (info.search == ""))
            {
                ...
                else
                {
                    ...
                    result = from p in result
                             where p.RESPONSABLE.ToUpper().Contains(info.search) || p.DETALLE.ToUpper().Contains(info.search) || p.DETALLE_ITEM.ToUpper().Contains(info.search)||p.DETALLE_MATERIAL.ToUpper().Contains(info.search)||p.NRO_ORDEN.ToUpper().Contains(info.search)
                             select p;
                }
                Total = result.Count();
