            ...
            if ((info.search == null) || (info.search == ""))
            {
                ...
                else
                {
                    ...
                    result = from p in result
                             where p.NOMBRE.ToUpper().Contains(info.search) || p.CODIGO.ToUpper().Contains(info.search) || p.CALIDAD.ToUpper().Contains(info.search) || p.CATEGORIA.ToUpper().Contains(info.search) || p.COLOR.ToUpper().Contains(info.search)
                             select p;
                }
                Total = result.Count();
