        ...
        public DataPaged<EE_DETALLES_DES_MAT> GetAllMateriales(PagingInfo info, string Codigo)
        {
            ...
            else
            {
                ...
                else
                {
                    info.search = info.search.ToUpper();
                    result = from p in result
                             where p.RESPONSABLE.ToUpper().Contains(info.search) || p.DETALLE.ToUpper().Contains(info.search)|| p.OBSERVACION.ToUpper().Contains(info.search)
                             select p;
                }
                Total = result.Count();
                //result = QueryPaged(result, info.limit, info.start, info.sort, info.dir);
        ...
        public DataPaged<EE_DETALLES_DES_ENTREGADO> GetAllEntregados(PagingInfo info, string Codigo)
        {
            ...
            else
            {
                ...
                else
                {
                    info.search = info.search.ToUpper();
                    result = from p in result
                             where p.RESPONSABLE.ToUpper().Contains(info.search) || p.OBSERVACION.ToUpper().Contains(info.search)
                             select p;
                }
                Total = result.Count();
                //result = QueryPaged(result, info.limit, info.start, info.sort, info.dir);
