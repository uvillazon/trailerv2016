...
{
    public class EmpresasManager : Repository<EE_EMPRESAS>
    {


        public EmpresasManager(IUnitOfWork uow) : base(uow) { }
        ...
        public DataPaged<EE_EMPRESAS> GetAllEmpresas(PagingInfo info, string Codigo)
        {
            ...
            IQueryable<EE_EMPRESAS> result = null;
            int Total = 0;
            if ((info.search == null) || (info.search == ""))
            {
                ...
                result = from p in context.EE_EMPRESAS
                         where p.NOMBRE.ToUpper().Contains(info.search) || p.CODIGO.ToUpper().Contains(info.search) || p.CIUDAD.ToUpper().Contains(info.search)
                             select p;
                Total = result.Count();
                result = QueryPaged(result, info.limit, info.start,info.sort,info.dir);
               
            }
            var resultado = new DataPaged<EE_EMPRESAS>()
            {
                Page = info.page,
                Records = info.limit,
                Rows = result.ToList(),
