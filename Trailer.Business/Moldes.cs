        ...
        public DataPaged<EE_MOLDES> GetAllMoldes(PagingInfo info, string Codigo)
        {
            ...
            if ((info.search == null) || (info.search == ""))
            {
                ...
                result = from p in context.EE_MOLDES
                             where p.NRO_MOLDE.ToUpper().Contains(info.search)||p.EMPRESA.ToUpper().Contains(info.search)||p.CATEGORIA.ToUpper().Contains(info.search)||p.TALLA.ToUpper().Contains(info.search)
                             select p;
                Total = result.Count();
