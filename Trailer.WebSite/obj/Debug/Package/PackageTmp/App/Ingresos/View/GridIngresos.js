Ext.define("App.Ingresos.View.GridIngresos", {
    ...
    initComponent: function () {
        ...
        this.columns = [
            ...
            { header: "Nro Ingreso", width: 100, sortable: true, dataIndex: 'NRO_INGRESO' },
            ...
            { header: "Fecha <br> Ingreso", width: 100, sortable: true, dataIndex: 'FECHA_INGRESO', renderer: Ext.util.Format.dateRenderer('d/m/Y') },
        ...
        if (data != null) {
            Ext.MessageBox.confirm('Confirmacion?', 'Esta Seguro de Contabilizar este Ingreso?', function (btn) {
                if (btn == 'yes') {
                    me.el.mask('Procesando...', 'x-mask-loading');
                    Ext.Ajax.request({
                        url: '../Ingresos/ContabilidadIngreso',
                        params: { ID_INGRESO: data.get('ID_INGRESO') },
                        success: function (response) {
                            me.el.unmask();
                            r = Ext.decode(response.responseText);
