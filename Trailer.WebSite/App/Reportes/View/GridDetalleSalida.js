Ext.define("App.Salidas.View.GridDetalleSalida", {
    ...
    initComponent: function () {
        ...
        var me = this;
        me.store = Ext.create('App.Salidas.Store.DetallesSalida');
...
//        alert('asdasd');
        ...
        if (!Ext.isEmpty(modified)) {
            ...
            Ext.MessageBox.confirm('Confirmacion?', 'Esta Seguro de Guardar Los Cambios?', function (btn) {
                if (btn == 'yes') {
                    ...
                    Ext.Ajax.request({
                        url: host+'Salidas/CrearDetallesSalida',
    ...
    Eliminar: function () {
        ...
        if (data != null) {
            if (data.get('ID_DETALLE_SALIDA') != 0) {
                alert(data.get('ID_DETALLE_SALIDA'));
                Ext.MessageBox.confirm('Confirmacion?', 'Esta Seguro de Eliminar este Registro?', function (btn) {
                    if (btn == 'yes') {
                        me.el.mask('Procesando...', 'x-mask-loading');
                        Ext.Ajax.request({
                            url: host+'Salidas/EliminarDetalleSalida',
                            params: { ID_DETALLE_SALIDA: data.get('ID_DETALLE_SALIDA') },
